...
<div class="wrapper accaunt-container">
    <div class="container">
      <section class="accaunt">
        ...
        <ul class="accaunt-plus__menu">
          <li id="all" class="<?=(!isset($_GET['class'])||$_GET['class']=='')?'active':''?>" onclick="window.location.href='/id<?=$model->id?>';">Все</li>
          <?php if(Avto::model()->countByAttributes(array('uid'=>$model->id,'class'=>'econom'))>0): ?>
          <li id="econom" class="<?=(isset($_GET['class'])&&$_GET['class']=='econom')?'active':''?>" onclick="window.location.href='/id<?=$model->id?>?class=econom';">Эконом</li>
          <?php endif; ?>
          
          <?php if(Avto::model()->countByAttributes(array('uid'=>$model->id,'class'=>'premium'))>0): ?>
          <li id="premium" class="<?=(isset($_GET['class'])&&$_GET['class']=='premium')?'active':''?>" onclick="window.location.href='/id<?=$model->id?>?class=premium';">Премиум</li>
          <?php endif; ?>
          
          <?php if(Avto::model()->countByAttributes(array('uid'=>$model->id,'class'=>'bus'))>0): ?>
          <li id="bus" class="<?=(isset($_GET['class'])&&$_GET['class']=='bus')?'active':''?>" onclick="window.location.href='/id<?=$model->id?>?class=bus';">Автобусы</li>
          <?php endif; ?>
        </ul>  
        <div class="accaunt-plus__service__list">
          ...
          <div class="accaunt-plus__service__item">
            <div class="accaunt-plus__service__item-img">
              <a href="/users/<?=$model->id?>/avto350_<?=$item->picture?>" rel="gallery1" class="avto__zoom"><img src="/users/<?=$model->id?>/<?=$item->picture?>" alt="<?=$model->name?> - <?=$item->title?>" /></a>
              <div style="margin-left: -10000px;width:1px;height:1px;">
              <?php
              foreach($cnt as $foto) {
                if($item->picture!=$foto->file)
                    echo '<a href="/users/'.$model->id.'/'.$foto->file.'" rel="gallery1" class="avto__zoom"><img src="/users/'.$model->id.'/'.$foto->file.'" /></a>';
              }
              ?>
              </div>
              <div class="accaunt-plus__img-number">
                <span><?=count($cnt)?> фотографий</span>
              </div>
            </div>
            <div class="accaunt-plus__service__item-info">
              <a href="#"><?=$item->title?></a>
              <p>
                <?=$item->year?> год<br>
                Количество дверей: <?=$item->doors?><br>
                Количество мест: <?=$item->seats?><br>
                Цвет: <?=$item->color?>
              </p>
            </div>
            <div class="accaunt-plus__service__item-text">
            <h6>Дополнительные условия:</h6>
            <div class="accaunt-plus__service__item-text-scroll nano has-scrollbar">
              <div class="accaunt-plus__service__item-text-content nano-content">
                
                <?=$item->description?>
              </div>
            </div>
            </div>
            <div class="accaunt-plus__service__item-price">
              Стоимость:
              <span><?=$item->price?> грн/час</span>
            </div>
          </div>
          <?php
          }
