  ...
  <div class="wrapper wrapper-cabinet">
    <div class="container clfx">
      ...
      <div class="clfx"></div>
        ...
        <div class="cabinet-pro__profile">
          <div id="cabiner__gallery__select">
               <!--h3 class="cabinet__accordion__title">слайдер</h3>
              <div class="cabinet__video__list">
                <div class="col-8 cabinet__portfolio__select4-photo"><a href="#">Максимальное количество баннеров для слайдера - 3 шт</a></div>
                <div class="col-4 global__plus__btn-container">
                        <?php
                        if(!is_object($banners) || ($banners->banner1=='' || $banners->banner2=='' || $banners->banner3=='')) {
                            $this->widget('ext.Upload.Upload',
                            array(
                                    'id'=>'uploadFiles',
                                    'config'=>array(
                                           'action'=>Yii::app()->createUrl('/my/studio/uploadbanner'),
                                           'allowedExtensions'=>array("jpg","jpeg","gif","png"),//array("jpg","jpeg","gif","exe","mov" and etc...
                                           'sizeLimit'=>8*1024*1024,// maximum file size in bytes
                                           'minSizeLimit'=>1*1024,
                                           'auto'=>true,
                ...
                if(count($banners)>0) {
                    ...
                    {
                ?>
                    <figure class="account__slider__img">
                      <a href="/my/studio/delbanner/id/<?=$banners->id?>_1" class="cabinet__photo__item-delete show__tooltip" data-toggle="tooltip" data-placement="bottom" data-title="удалить"></a>
                      <img src="/users/<?=$model->id?>/<?=$banners->banner1?>" alt="">
                    </figure>
                <?php        
                    }
                    else {
                ?>
                    <div class="account__sliter__size">Размер баннера 955Х278 px</div>
                <?php                        
                    }
                    if($banners->banner2!='')
                    {
                ?>
                    <figure class="account__slider__img">
                      <a href="/my/studio/delbanner/id/<?=$banners->id?>_2" class="cabinet__photo__item-delete show__tooltip" data-toggle="tooltip" data-placement="bottom" data-title="удалить"></a>
                      <img src="/users/<?=$model->id?>/<?=$banners->banner2?>" alt="">
                    </figure>
                <?php        
                    }
                    else {
                ...
                ?>
                    <figure class="account__slider__img">
                      <a href="/my/studio/delbanner/id/<?=$banners->id?>_3" class="cabinet__photo__item-delete show__tooltip" data-toggle="tooltip" data-placement="bottom" data-title="удалить"></a>
                      <img src="/users/<?=$model->id?>/<?=$banners->banner3?>" alt="">
                    </figure>
                <?php        
                    }
                    else {
            ...
            <h3 class="cabinet__accordion__title">ЗАЛЫ</h3>
              <div class="cabinet__photo__list">
                <div class="col-9 cabinet__portfolio__select4-photo"><a href="#" style="display:none;">Выберите любые 4-е фотографии из любых альбомов для главной страницы</a></div>
                <div class="col-3 cabinet__portfolio__add-photo"><!--a href="/my/profile" class="add-cabinet__btn" data-toggle="modal" data-target="#__add-albom">Добавить зал</a--></div>
                <div class="clfx"></div>
                <?php 
                if(count($halls) > 0):
                    $cnt=count($halls);
                ...
                ?>
                      ...
                      <figure class="cabinet__photo__item">
                        <a href="/my/studio/album/id/<?=$item->id?>">
                        <?php if($item->picture!=''): ?>
                        <img src="/users/<?=Yii::app()->user->id?>/254_<?=$item->picture?>" alt="" />
                        <?php else: ?>
                        <img src="/img/zaglushka.png" alt="" />
                        <?php endif; ?>
                        <a href="/my/studio/visiblealbum/id/<?=$item->id?>" class="cabinet__photo__item-view"></a>
                        <a href="/my/studio/album/id/<?=$item->id?>" class="cabinet__photo__item-edit"></a>
                        ...
                        <figcaption class="accaunt-gallery__thumbnail-overlay cabinet-gallery__thumbnail-overlay">
                          <a href="#">
                            <div class="accaunt-gallery__info-container"><?=$item->title?><div class="accaunt-gallery__info-number"><?=count($item->files);?></div></div>
                          </a>
                        </figcaption>
                        <?php
                          if($item->visible==0): 
                      ...
                      <script>
                        $('#<?=$item->id?>').click(function(){
                            $('.delete__hidden').hide();
                            $.ajax({
                                url: '/my/studio/delalbum/id/<?=$item->id.'?back_url='.Yii::app()->request->requestUri?>',          
                                type : "get",                     
                ...
                <?php        
                    ...
                    ?>
                    <figure class="cabinet__photo__item">
                      <a href="/my/profile"><img src="/img/zaglushka.png" alt="" /></a>
                      <figcaption class="accaunt-gallery__thumbnail-overlay cabinet-gallery__thumbnail-overlay">
                        <a href="">
                          <div class="accaunt-gallery__info-container"></div>
                        </a>
                      </figcaption>
                    </figure>
                    <?php    
                    ...
                    echo Yii::t('flo','No results found.');
                endif;
                ?>
              </div>
          </div>
