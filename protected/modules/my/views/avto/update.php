      ...
      <div class="clfx"></div>
        ...
        <div class="cabinet-plus__auto-park">
          <header class="cabinet__portfolio__title clfx">
            ...
            <div class="col-4"><h3>Редактировать автопарк</h3></div>
            <div class="col-4"></div>
          </header>
          <?php
          if(isset($_GET['new'])) {
          ...
          <div class="cabinet__portfolio__title clfx" style="margin-top: 20px; padding: 20px;">
            <div class="col-6" align="right" style="padding-right: 20px;">1. Добавление авто</div>
            <div class="col-6" align="left" style="padding-left: 20px;"><strong>2. Добавление фото</strong></div>
          </div>
          <?php } ?>
          <div class="col-12 global__plus__btn-container border__bottom-grey">
            <?php
                ...
                array(
                        'id'=>'uploadFiles',
                        'config'=>array(
                               'action'=>Yii::app()->createUrl('/my/avto/upload/id/'.$_GET['id']),
                               'allowedExtensions'=>array("jpg","jpeg","gif","png"),//array("jpg","jpeg","gif","exe","mov" and etc...
                               'sizeLimit'=>8*1024*1024,// maximum file size in bytes
                               'minSizeLimit'=>1*1024,
                               'auto'=>true,
                               ...
                               'onComplete'=>'js:function(id, fileName, responseJSON){
                                    if (responseJSON.success)
                                    {
                                        ...
                                        $.ajax({
                                            ...
                                            cache: false,
                                            type: "POST",
                                            data: {id: responseJSON.res, b_url:"'.Yii::app()->request->requestUri.'", m_id:'.$_GET['id'].'},
          ...
          <div class="col-4 plus__add__auto-park__img-container" id="photos">
            ...
            if(count($files)>0) {
                foreach($files as $item) {
                    if($model->picture==$item->file) $chk='checked';
                    else $chk='';
                    echo '<figure class="plus__add__auto-park__img">
                              <a href="#"><img src="/users/'.Yii::app()->user->id.'/'.$item->file.'" alt="" /></a>
                                <input type="radio" id="avto_cover_'.$item->id.'" name="avto_cover" '.$chk.' value="'.$item->file.'" onClick="setCover('.$model->id.',\''.$item->file.'\');">
                                <label for="avto_cover_'.$item->id.'">Обложка на главной</label>
                                '.CHtml::link('удалить фото','/my/avto/deletefile/id/'.$item->id.'/pid/'.$_GET['id'].'?back_url='.Yii::app()->request->requestUri,array('confirm'=>'Точно хотите удалить?')).'
                            </figure>';
                }
            }
            ?>
