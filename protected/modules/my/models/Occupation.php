...
class Occupation extends CActiveRecord
{
	...
	public function relations()
	{
	   $city=Yii::app()->request->cookies['city']->value;
		...
		return array(
            ...
            'usersCity'=>array(self::HAS_MANY, 'Users', 'occupation_id', 'condition'=>'city_id="'.$city.'" or other_city="'.$city.'"'),
		);
	}
    ...
    public function getName($id)
    {
        ...
        if(count($data) > 0) return $data->name;
        else return false;
    }
    ...
    {
        $city=Yii::app()->request->cookies['city']->value;
        $users=Users::model()->findAllBySql('select id from {{users}} where (city_id="'.$city.'" or other_city="'.$city.'") and activate=1 and occupation_id='.$id);
        $i=0;
        foreach($users as $user) {
            if($id==1 || $id==2 || $id==4 || $id==18 || $id==19) {
